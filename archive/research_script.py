...
# Read config.json (for reference, though not heavily used)
...
for ticker in tickers:
    ...
    # Format price change
    price_str = f"${current:.2f}" if current is not None else "N/A"
    ...
    for ticker in ticker_list[:3]:  # Limit to 3 tickers per theme to avoid too many requests
        ...
        for item in items:
            ...
            if theme_name == 'water' and any(word in title_lower for word in ['water', 'utility', 'utility']):
                news_items.append(f"{ticker}: {item['title'][:60]}")
...
lines.append("- All takeaways based on headline keyword analysis are unverified.")
